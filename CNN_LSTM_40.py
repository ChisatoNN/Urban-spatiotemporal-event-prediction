...
class MyNet(nn.Module):
    def __init__(self, image_width=image_width, image_height=image_height):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(in_features=9 * 9 * 128, out_features=32 * 32, bias=True),  # 像素64 in_features=8*8*256
            nn.LeakyReLU(),

        )

        self.lstm = nn.LSTM(32 * 32, 32 * 32, num_layers=2, bidirectional=False,
                            batch_first=True, )

        self.fc2 = nn.Sequential(
            nn.Linear(in_features=32 * 32, out_features=image_width * image_height, bias=True),
            nn.LeakyReLU(),
        )
...
def calculate_indicater(TRUE_lable, PRE_lable):
    ...
    Sum_mean = np.mean(Sum_mean, 0)  # 真实平均
...
if __name__ == "__main__":
    ...
    modelpath = './model/model_40_lstm2.ckpt'
    loss_path = './loss/loss_40_lstm2.csv'
    Save_path = './save_data/save_data_40_lstm2.csv'
    Val_path = './save_data/val_data_40_lstm2.csv'
