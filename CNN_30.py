...
class MyNet(nn.Module):
    def __init__(self, image_width = image_width, image_height = image_height):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=6*6*128, out_features=64*64, bias=True), # 64 in_features=8*8*256
            nn.Dropout(0.2),  # drop 50% of the neuron
            nn.LeakyReLU(),
...
if __name__ == "__main__":
    ...
    path1 = './feature(30)/'  # picture-48
    train_x, train_y, target_name = read_picture(path1)
    modelpath = './model/model_30_2.ckpt'
    loss_path = './loss/loss_30_2.csv'
    Save_path = './save_data/save_data_30_2.csv'
    Val_path = './save_data/val_data_30_2.csv'
    predict_path = './CNN_30/'  
